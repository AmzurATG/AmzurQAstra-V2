"""Initial schema - all tables, enums, indexes, triggers, and seed data
...
def upgrade() -> None:
    ...
    CREATE INDEX IF NOT EXISTS idx_user_stories_integrity_check ON user_stories(project_id, integrity_check) WHERE integrity_check = TRUE;
    ...
    CREATE INDEX IF NOT EXISTS idx_test_cases_integrity_check ON test_cases(project_id, integrity_check) WHERE integrity_check = TRUE;
...
def downgrade() -> None:
    for t in ["test_results", "test_runs", "test_steps", "test_cases",
              "requirements", "audit_logs", "user_stories",
              "project_integrations", "projects", "users", "organizations"]:
        op.execute(f'DROP TABLE IF EXISTS "{t}" CASCADE;')

    op.execute("DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;")

    for e in ["userstoryitemtype", "userstorysource", "userstorypriority",
              "userstorystatus", "syncstatus", "integrationcategory",
              "integrationtype", "testresultstatus", "testrunstatus",
              "teststepaction", "testcasestatus", "testcasecategory",
              "testcasepriority", "requirementsourcetype", "userrole"]:
