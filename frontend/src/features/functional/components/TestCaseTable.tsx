...
function effectiveSource(tc: TestCase): 'manual' | 'ai' | 'csv' {
              ...
              <td className="px-4 py-4">
                <span
                  className={`inline-flex px-2 py-1 text-xs rounded font-medium ${
                    sourceStyles[effectiveSource(tc)] || sourceStyles.manual
                  }`}
                  title={
                    effectiveSource(tc) === 'csv'
                      ? 'Imported from CSV'
                      : effectiveSource(tc) === 'ai'
                        ? 'Generated by AI'
                        : 'Created manually'
                  }
                >
                  {sourceLabels[effectiveSource(tc)] ?? effectiveSource(tc)}
                </span>
              </td>
              <td className="px-4 py-4">
