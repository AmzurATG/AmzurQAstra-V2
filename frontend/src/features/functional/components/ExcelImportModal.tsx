...
const MAX_FILE_BYTES = 20 * 1024 * 1024 // 20 MiB — mirrors backend MAX_EXCEL_BYTES
...
interface ExcelImportModalProps {
  ...
  onClose: () => void
  projectId: number
  /** Called after a non–dry-run import. wroteCases is true when rows were inserted. */
...
export const ExcelImportModal: React.FC<ExcelImportModalProps> = ({
  ...
  const [importMode, setImportMode] = useState<'strict' | 'permissive'>('permissive')
  const [sprintPrefix, setSprintPrefix] = useState('')
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    ...
    for (const f of list) {
      ...
      if (!name.endsWith('.xlsx') && !name.endsWith('.xls')) {
        toast.error(`${f.name}: not an Excel file (.xlsx / .xls) — skipped`)
        continue
      }
      if (f.size > MAX_FILE_BYTES) {
        toast.error(`${f.name}: exceeds 20 MB limit — skipped`)
        continue
      }
  ...
  const runImport = async () => {
    ...
    if (pending.length === 0) {
      toast.error('Add at least one Excel file to import')
      return
    }
    setBusy(true)
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        fd.append('sprint_prefix', sprintPrefix.trim())
        fd.append('file', entry.file)
        const res = await testCasesApi.importExcel(fd)
        const data = res.data
    ...
    if (!dryRun) {
      ...
      if (anyWroteCases) {
        toast.success(
          `Excel import complete — ${totalCreatedCases} case(s) created.`
        )
      }
    } else {
                ...
                <div className="flex items-start justify-between gap-4 mb-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Upload a <strong>.xlsx</strong> or <strong>.xls</strong> workbook (max{' '}
                      <strong>20 MB</strong>). Each detail sheet maps to a project user story;
                      each row becomes one test case with <em>custom</em>-action steps.
                    </p>
                  </div>
                  <button
                    type="button"
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      value={sprintPrefix}
                      onChange={(e) => setSprintPrefix(e.target.value)}
                      placeholder="e.g. S1- or Sprint1-"
                      disabled={busy}
                      className="w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-primary-500 outline-none"
                    />
                    <p className="text-xs text-gray-400 mt-1">
                      Prepended to each case key (e.g. <code>S1-TC_001</code>) to avoid collisions
                      when importing multiple sprints into the same project.
                    </p>
                  </div>
                  ...
                  <div>
                    <p className="text-sm font-medium text-gray-700 mb-2">
                      Excel files{' '}
                      <span className="text-xs font-normal text-gray-400">(up to 20 MB each)</span>
                    </p>
                    <input
                      ref={fileInputRef}
                      type="file"
                    ...
                    <button
                      ...
                      className={`w-full rounded-xl border-2 border-dashed px-4 py-6 text-center transition-colors disabled:opacity-50 ${
                        dragActive
                          ...
                          : 'border-gray-300 bg-gray-50/50 hover:border-green-300 hover:bg-green-50/40'
                      }`}
                    >
                      <DocumentArrowUpIcon className="w-9 h-9 mx-auto text-gray-400 mb-2" />
                      <div className="text-sm text-gray-600">
                        <span className="font-medium text-green-600">Drop .xlsx / .xls files here</span>
                        {' · '}
                  ...
                  <label className="flex items-start gap-3 text-sm text-gray-700 cursor-pointer rounded-lg border border-gray-200 p-3 hover:bg-gray-50">
                    ...
                    <span>
                      ...
                      <span className="block text-gray-500 text-xs mt-0.5">
                        Check the workbook without inserting rows. Uncheck to import into the project.
                      </span>
                    </span>
                  </label>
                  ...
                  <div>
                    ...
                    >
                      <option value="permissive">
                        Permissive — skip unmapped stories; import the rest (recommended)
                      </option>
                      <option value="strict">
                        Strict — abort if any story cannot be matched
                      </option>
                    </select>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 justify-end pt-2">
                    ...
                    {allDone && !dryRun && (
                      <Button
                        ...
                        onClick={() =>
                          setFiles((prev) =>
                            prev.map((e) => ({ ...e, done: false, result: null, error: null }))
                          )
                        }
                        disabled={busy}
                      >
